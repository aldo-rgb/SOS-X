...
export function parseDeepLink(url: string | null | undefined): DeepLinkTarget | null {
  ...
  try {
    const s = String(url);
    // /cotizar no lleva TRN: abre la sección Cotizar (QuoteHub).
...
export function emitDeepLink(target: DeepLinkTarget | null): void {
  // 'xpay' y 'cotizar' no requieren TRN; el resto sí.
  if (!target || (target.action !== 'xpay' && target.action !== 'cotizar' && !target.trn)) return;

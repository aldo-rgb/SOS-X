/**
 * ConfirmDeliveryScreen — Confirmación de entrega de un contenedor.
 * El monitorista debe subir 3 fotos para marcar el contenedor como entregado.
 */
...
export default function ConfirmDeliveryScreen({ navigation, route }: any) {
  ...
  const [photos, setPhotos] = useState<Record<SlotKey, string | null>>({ photo1: null, photo2: null, photo3: null });
  ...
  const submit = async () => {
    if (!photos.photo1 || !photos.photo2 || !photos.photo3) {
      Alert.alert('Faltan fotos', 'Debes subir las 3 fotos para confirmar la entrega.');
      return;
    }
    if (!container?.id) {
    ...
    try {
      ...
      fd.append('photo3', buildFile(photos.photo3!, 'photo3'));

      const resp = await fetch(`${API_URL}/api/monitoreo/containers/${container.id}/confirm-delivery`, {
        method: 'POST',
      ...
      Alert.alert('✅ Entrega confirmada', 'El contenedor se marcó como entregado.', [
  ...
  const ready = photos.photo1 && photos.photo2 && photos.photo3 && !submitting;
  ...
  return (
    <SafeAreaView style={styles.container}>
      ...
      <ScrollView contentContainerStyle={{ padding: 16, paddingBottom: 40 }}>
        ...
        <Text style={styles.sectionTitle}>📸 Sube las 3 fotos de entrega</Text>
        <Text style={styles.sectionHint}>
          Las 3 fotos son obligatorias. Una vez confirmada la entrega, el contenedor pasa a “Entregado”.
        </Text>
        ...
        <TouchableOpacity
          style={[styles.submitBtn, !ready && { opacity: 0.5 }]}
          disabled={!ready}
          onPress={submit}
        >
          {submitting ? (
            ...
            <>
              ...
              <Text style={styles.submitText}>Confirmar entrega</Text>
            </>
          )}
        </TouchableOpacity>
      </ScrollView>
...
const styles = StyleSheet.create({
  ...
  submitBtn: {
    flexDirection: 'row', alignItems: 'center', justifyContent: 'center', gap: 8,
    backgroundColor: GREEN, borderRadius: 10, paddingVertical: 14, marginTop: 8,
  },

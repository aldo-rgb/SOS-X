  ...
  const getStatusLabel = (status: string, shipmentType?: string, receivedBy?: string | null): string => {
    ...
    if (shipmentType === 'china_air') {
      const chinaAirLabels: Record<string, string> = {
        ...
        in_transit_mx:       L('En Ruta a CEDIS',                'In Transit to Warehouse',         '运往仓库途中'),
        received_cedis:      L('En CEDIS',                       'At Warehouse',                    '在仓库'),
        received_cdmx:       L('En CEDIS CDMX',                  'At CDMX Warehouse',               '在墨西哥城仓库'),
        received_cdx:        L('En CEDIS CDMX',                  'At CDMX Warehouse',               '在墨西哥城仓库'),
        received_mty:        L('En CEDIS MTY',                   'At MTY Warehouse',                '在蒙特雷仓库'),
        customs_cleared:     L('Liberado de Aduana',             'Customs Cleared',                 '清关完成'),
        in_transit_transfer: L('En Tránsito',                    'In Transit',                      '运输中'),
        ready_pickup:        L('Listo para Recoger',             'Ready for Pickup',                '可取货'),
        delivered:           L('Entregado',                      'Delivered',                       '已送达'),
    ...
    <View style={styles.container}>
      ...
      {selectedIds.length > 0 && (() => {
        ...
        const totalToPay = selectedPackages.reduce((sum, p) => {
          ...
          // Fallback a saldo_pendiente / assigned_cost_mxn si nada en BD aún (paquete sin costear)

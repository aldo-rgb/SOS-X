  ...
  try {
    ...
    const pkgSelect = `
      SELECT 
        ...
        CASE WHEN COALESCE(NULLIF(p.saldo_pendiente, 0), NULLIF(p.pobox_service_cost, 0), NULLIF(p.air_sale_price, 0), NULLIF(p.pobox_venta_usd, 0), 0) = 0 AND COALESCE(p.monto_pagado, 0) > 0 THEN true ELSE false END as client_paid,

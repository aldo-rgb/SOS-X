...
export const getAdvisorShipments = async (req: Request, res: Response): Promise<any> => {
  try {
    ...
    if (!serviceType || serviceType === 'all') {
      ...
      // Solo masters/standalone (NO hijas): la master agrupa sus guías y muestra
      // "N guías" (las hijas se ven al expandir). Se mantiene master_id IS NULL de
      // pkgSelect para no listar TDX-...-001/-002/-003 junto a su master.
    ...
    if (prefix === 'PKG') {
      const r = await pool.query(
        `SELECT p.id,
                ...
                p.description,
                COALESCE(p.weight, 0) AS weight,
                COALESCE(p.air_chargeable_weight, 0) AS air_chargeable_weight,
                COALESCE(p.pkg_length, p.long_cm, 0) AS length_cm,
                COALESCE(p.pkg_width, p.width_cm, 0) AS width_cm,
                COALESCE(p.pkg_height, p.height_cm, 0) AS height_cm,
                p.image_url,
      ...
      row = {
        ...
        // CW (peso cobrable) — para Aéreo/TDI, es lo que se cobra (mayor entre real y volumétrico).
        air_chargeable_weight: parseFloat(p.air_chargeable_weight) || null,
        air_source: p.air_source || null,

  ...
  try {
    ...
    const pkgSelect = `
      SELECT 
        ...
        COALESCE(p.assigned_cost_mxn, p.saldo_pendiente, p.pobox_service_cost, p.air_sale_price, p.pobox_venta_usd, 0) as monto,
        CASE WHEN COALESCE(p.saldo_pendiente, p.pobox_service_cost, p.air_sale_price, p.pobox_venta_usd, 0) = 0 AND COALESCE(p.monto_pagado, 0) > 0 THEN true ELSE false END as client_paid,

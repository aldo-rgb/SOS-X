...
async function notify(userId: number | null, title: string, message: string, data: any = {}, notificationType: string = 'task_new'): Promise<void> {
  ...
  try {
    ...
    // laboral (10:10am–6pm) — salvo admins/super_admins, que reciben siempre (24/7,
    // incluidos fines de semana). El in-app ya quedó creado arriba.
    const workHoursGated = notificationType === 'task_new' || notificationType === 'task_comment';
    const recipients = await filterRecipientsForPush([userId], workHoursGated);
    if (recipients.length) {
      await sendPushToUsers(recipients, { title, body: message, data: { screen: 'MyTasks', ...data }, notificationType });
    }
  } catch { /* opcional */ }
}
        ...
        await pool.query(
          `UPDATE task_schedules
              SET last_task_id = $2,
                  next_run_at = (
                    SELECT MAX(ts) FROM (
                      SELECT generate_series(next_run_at, NOW() + ${step}, ${step}) AS ts
                    ) g WHERE ts > NOW()
                  )
            WHERE id = $1`, [sch.id, taskId]);
      }
    }
    if (due.rows.length) console.log(`[tasks] programadas: ${due.rows.length} tarea(s) generadas`);
...
export const deleteSchedule = async (req: Request, res: Response): Promise<any> => {
  try {
    const uid = authUserId(req);
    const id = parseInt(String(req.params.id));
    await pool.query(`UPDATE task_schedules SET active = FALSE WHERE id = $1 AND (created_by = $2 OR $3)`, [id, uid, isManager(req)]);
    res.json({ success: true });
  } catch (e: any) {
    console.error('[tasks] deleteSchedule:', e); res.status(500).json({ error: 'Error al eliminar programada' });
  }
};
  ...
  try {
    ...
    const t = await pool.query(`
      SELECT t.*, u.full_name AS assignee_name, u.referral_code AS assignee_referral_code,
        ...
        LEFT JOIN task_columns col ON col.id = t.column_id
        LEFT JOIN task_boards bd ON bd.id = t.board_id
       WHERE t.id = $1`, [id]);

        ...
        res.json({
            ...
            shipment: {
                master: { id: pkg.id, tracking: (pkg.child_no && /^AIR/i.test(String(pkg.child_no))) ? pkg.child_no : pkg.tracking_internal, trackingProvider: pkg.tracking_provider || aggregatedOriginGuides,
                    ...
                    gexInsuranceCost: pkg.gex_insurance_cost != null ? parseFloat(pkg.gex_insurance_cost) : null,
                    gexFixedCost: pkg.gex_fixed_cost != null ? parseFloat(pkg.gex_fixed_cost) : null,
                    declaredValueMxn: pkg.declared_value_mxn != null ? parseFloat(pkg.declared_value_mxn) : null,
...
const getChinaAirStatusLabel = (status: string): string => {
    const labels: Record<string, string> = {
        ...
        'received_cedis': '✅ En CEDIS',
        'received_cdmx': '🏢 En CEDIS CDMX',
        'received_cdx': '🏢 En CEDIS CDMX',
        'received_mty': '✅ En CEDIS MTY',
    ...
    try {
        ...
        const dhlPackages = dhlResult.rows.map(pkg => {
            ...
            return {
                ...
                exchange_rate: pkg.exchange_rate ? parseFloat(pkg.exchange_rate) : null,
        ...
        const fclPackages = fclResult.rows.map(container => {
            ...
            const getFclStatusLabel = (status: string): string => {
                const labels: Record<string, string> = {
                    ...
                    'cbp_signature_received': '🖊️ Firma Electrónica CBP Recibida',
    ...
    try {
        ...
        const result = await pool.query(`
            SELECT 
                ...
                p.has_gex,
                p.gex_folio,
                p.gex_insurance_cost,
                p.gex_fixed_cost,
                p.gex_total_cost,
                p.declared_value_mxn,
                p.assigned_cost_mxn,
                p.saldo_pendiente,
                p.monto_pagado,
        ...
        const packageDetail = {
            ...
            gex_fixed_cost: pkg.gex_fixed_cost != null ? parseFloat(pkg.gex_fixed_cost) : null,
            gex_total_cost: pkg.gex_total_cost != null ? parseFloat(pkg.gex_total_cost) : null,
            declared_value_mxn: pkg.declared_value_mxn != null ? parseFloat(pkg.declared_value_mxn) : null,
            assigned_cost_mxn: pkg.assigned_cost_mxn ? parseFloat(pkg.assigned_cost_mxn) : 0,
            saldo_pendiente: pkg.saldo_pendiente ? parseFloat(pkg.saldo_pendiente) : 0,
            monto_pagado: pkg.monto_pagado ? parseFloat(pkg.monto_pagado) : 0,
                client_paid: pkg.client_paid || false,

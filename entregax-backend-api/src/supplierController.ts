        ...
        const result = await pool.query(`
            ...
            LEFT JOIN (
                SELECT
                    ...
                    COALESCE(SUM(COALESCE(pobox_provider_cost_mxn, pobox_provider_cost_usd * registered_exchange_rate, 0)), 0) as total_cost
                FROM packages
    ...
    try {
        ...
        const result = await pool.query(`
            SELECT 
                ...
                COALESCE(SUM(COALESCE(p.pobox_provider_cost_mxn, COALESCE(p.pobox_provider_cost_usd, p.pobox_cost_usd) * p.registered_exchange_rate, 0)) FILTER (WHERE COALESCE(p.missing_on_arrival, FALSE) = FALSE AND COALESCE(p.is_lost, FALSE) = FALSE AND COALESCE(p.costing_paid, FALSE) = FALSE AND (p.status::text LIKE 'received%' OR p.status::text IN ('delivered', 'out_for_delivery'))), 0) as total_cost_mxn,
                COALESCE(SUM(COALESCE(p.pobox_provider_cost_usd, p.pobox_cost_usd)) FILTER (WHERE COALESCE(p.missing_on_arrival, FALSE) = FALSE AND COALESCE(p.is_lost, FALSE) = FALSE AND COALESCE(p.costing_paid, FALSE) = FALSE AND (p.status::text LIKE 'received%' OR p.status::text IN ('delivered', 'out_for_delivery'))), 0) as total_cost_usd,
                -- Total YA PAGADO (no missing + no lost + paid + received/delivered/out_for_delivery)
                COALESCE(SUM(COALESCE(p.pobox_provider_cost_mxn, COALESCE(p.pobox_provider_cost_usd, p.pobox_cost_usd) * p.registered_exchange_rate, 0)) FILTER (WHERE COALESCE(p.missing_on_arrival, FALSE) = FALSE AND COALESCE(p.is_lost, FALSE) = FALSE AND COALESCE(p.costing_paid, FALSE) = TRUE AND (p.status::text LIKE 'received%' OR p.status::text IN ('delivered', 'out_for_delivery'))), 0) as paid_cost_mxn,
                COALESCE(SUM(COALESCE(p.pobox_provider_cost_usd, p.pobox_cost_usd)) FILTER (WHERE COALESCE(p.missing_on_arrival, FALSE) = FALSE AND COALESCE(p.is_lost, FALSE) = FALSE AND COALESCE(p.costing_paid, FALSE) = TRUE AND (p.status::text LIKE 'received%' OR p.status::text IN ('delivered', 'out_for_delivery'))), 0) as paid_cost_usd,
                ...
                COALESCE(SUM(COALESCE(p.pobox_provider_cost_mxn, COALESCE(p.pobox_provider_cost_usd, p.pobox_cost_usd) * p.registered_exchange_rate, 0)) FILTER (WHERE COALESCE(p.missing_on_arrival, FALSE) = FALSE AND COALESCE(p.is_lost, FALSE) = FALSE AND (p.status::text LIKE 'received%' OR p.status::text IN ('delivered', 'out_for_delivery'))), 0) as all_cost_mxn,
                COALESCE(SUM(COALESCE(p.pobox_provider_cost_usd, p.pobox_cost_usd)) FILTER (WHERE COALESCE(p.missing_on_arrival, FALSE) = FALSE AND COALESCE(p.is_lost, FALSE) = FALSE AND (p.status::text LIKE 'received%' OR p.status::text IN ('delivered', 'out_for_delivery'))), 0) as all_cost_usd
            FROM consolidations c
            JOIN packages p ON p.consolidation_id = c.id
    ...
    try {
        ...
        const result = await pool.query(`
            SELECT DISTINCT
                ...
                c.created_at,
                COUNT(p.id) as package_count,
                COALESCE(SUM(COALESCE(p.pobox_provider_cost_mxn, p.pobox_provider_cost_usd * p.registered_exchange_rate, 0)), 0) as total_cost_mxn,

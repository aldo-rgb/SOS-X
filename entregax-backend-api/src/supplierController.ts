    ...
    try {
        ...
        const result = await pool.query(`
            SELECT 
                ...
                COALESCE(SUM(COALESCE(p.pobox_provider_cost_mxn, COALESCE(p.pobox_provider_cost_usd, p.pobox_cost_usd) * p.registered_exchange_rate, 0)) FILTER (WHERE COALESCE(p.missing_on_arrival, FALSE) = FALSE AND COALESCE(p.is_lost, FALSE) = FALSE AND (p.status::text LIKE 'received%' OR p.status::text IN ('delivered', 'out_for_delivery', 'returned_to_warehouse'))), 0) as all_cost_mxn,
                COALESCE(SUM(COALESCE(p.pobox_provider_cost_usd, p.pobox_cost_usd)) FILTER (WHERE COALESCE(p.missing_on_arrival, FALSE) = FALSE AND COALESCE(p.is_lost, FALSE) = FALSE AND (p.status::text LIKE 'received%' OR p.status::text IN ('delivered', 'out_for_delivery', 'returned_to_warehouse'))), 0) as all_cost_usd
            FROM consolidations c
            JOIN packages p ON p.consolidation_id = c.id

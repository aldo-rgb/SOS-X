    ...
    try {
        ...
        const result = await pool.query(`
            SELECT 
                ...
                -- Total PENDIENTE de pago AHORA (unpaid + no missing + no lost + not in_transit)
                COALESCE(SUM(p.pobox_service_cost) FILTER (WHERE COALESCE(p.missing_on_arrival, FALSE) = FALSE AND COALESCE(p.is_lost, FALSE) = FALSE AND COALESCE(p.costing_paid, FALSE) = FALSE AND p.status::text NOT LIKE 'in_transit%'), 0) as total_cost_mxn,
                COALESCE(SUM(p.pobox_cost_usd) FILTER (WHERE COALESCE(p.missing_on_arrival, FALSE) = FALSE AND COALESCE(p.is_lost, FALSE) = FALSE AND COALESCE(p.costing_paid, FALSE) = FALSE AND p.status::text NOT LIKE 'in_transit%'), 0) as total_cost_usd,
                -- Total YA PAGADO (no missing + no lost + paid + not in_transit)
                COALESCE(SUM(p.pobox_service_cost) FILTER (WHERE COALESCE(p.missing_on_arrival, FALSE) = FALSE AND COALESCE(p.is_lost, FALSE) = FALSE AND COALESCE(p.costing_paid, FALSE) = TRUE AND p.status::text NOT LIKE 'in_transit%'), 0) as paid_cost_mxn,
                COALESCE(SUM(p.pobox_cost_usd) FILTER (WHERE COALESCE(p.missing_on_arrival, FALSE) = FALSE AND COALESCE(p.is_lost, FALSE) = FALSE AND COALESCE(p.costing_paid, FALSE) = TRUE AND p.status::text NOT LIKE 'in_transit%'), 0) as paid_cost_usd,
                ...
                -- Total COMPLETO de la consolidación (todo no-in-transit excepto perdidas/faltantes)
                COALESCE(SUM(p.pobox_service_cost) FILTER (WHERE COALESCE(p.missing_on_arrival, FALSE) = FALSE AND COALESCE(p.is_lost, FALSE) = FALSE AND p.status::text NOT LIKE 'in_transit%'), 0) as all_cost_mxn,
                COALESCE(SUM(p.pobox_cost_usd) FILTER (WHERE COALESCE(p.missing_on_arrival, FALSE) = FALSE AND COALESCE(p.is_lost, FALSE) = FALSE AND p.status::text NOT LIKE 'in_transit%'), 0) as all_cost_usd
            FROM consolidations c
            JOIN packages p ON p.consolidation_id = c.id

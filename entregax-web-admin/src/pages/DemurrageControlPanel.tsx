// ============================================
// PANEL DE CONTROL DE DEMORAS (DEMURRAGE)
// ============================================
...
export default function DemurrageControlPanel() {
    ...
    const fetchReferenciasValidas = useCallback(async () => {
        try {
            const res = await axios.get(`${API_URL}/api/demora/referencias/validas`, { headers: { Authorization: `Bearer ${getToken()}` } });
    ...
    const fetchStats = useCallback(async () => {
        try {
            const res = await axios.get(`${API_URL}/api/demora/stats`, { headers: { Authorization: `Bearer ${getToken()}` } });
            setStats(res.data);
        } catch { /* silenciar */ }
    }, []);
    ...
    const fetchProveedores = useCallback(async () => {
        try {
            const res = await axios.get(`${API_URL}/api/demora/proveedores`, { headers: { Authorization: `Bearer ${getToken()}` } });
    ...
    const fetchBolsas = useCallback(async (proveedorId?: number) => {
        try {
            const url = proveedorId ? `${API_URL}/api/demora/bolsas?proveedor_id=${proveedorId}` : `${API_URL}/api/demora/bolsas`;
    ...
    const fetchReferenciasBolsa = async (bolsaId: number) => {
        try {
            const res = await axios.get(`${API_URL}/api/demora/bolsas/${bolsaId}/referencias`, { headers: { Authorization: `Bearer ${getToken()}` } });
    ...
    const handleCreateProveedor = async () => {
        ...
        setSaving(true);
        try {
            await axios.post(`${API_URL}/api/demora/proveedores`, newProveedor, { headers: { Authorization: `Bearer ${getToken()}` } });
    ...
    const handleCreateBolsa = async () => {
        ...
        try {
            ...
            await axios.post(`${API_URL}/api/demora/bolsas`, formData, { headers: { Authorization: `Bearer ${getToken()}` } });
            setSnackbar({ open: true, message: `✅ Pago de demora $${formatCurrency(getTotalReferencias())} registrado`, severity: 'success' });
    ...
    const handleDeleteBolsa = async () => {
        ...
        setDeleting(true);
        try {
            await axios.delete(`${API_URL}/api/demora/bolsas/${bolsaToDelete.id}`, { headers: { Authorization: `Bearer ${getToken()}` } });
    ...
    return (
        <Box sx={{ p: 3 }}>
            ...
            <Box sx={{ mb: 3, display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: 2 }}>
                <Box>
                    <Typography variant="h4" fontWeight="bold" sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
                        <DemurrageIcon sx={{ color: THEME_COLOR }} />
                        Control de Demoras
                    </Typography>
                ...
                <Grid container spacing={2} sx={{ mb: 3 }}>
                    ...
                    <Grid size={{ xs: 12, sm: 4 }}>
                        <Card sx={{ bgcolor: '#FCE4EC', height: '100%' }}>
                            <CardContent sx={{ textAlign: 'center', py: 2, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', height: '100%' }}>
                                <ReceiptIcon sx={{ fontSize: 30, color: '#AD1457' }} />
            ...
            {tabValue === 0 && (
                <Box>
                    <Box sx={{ display: 'flex', justifyContent: 'space-between', mb: 2 }}>
                        <Typography variant="h6">Proveedores de Demora</Typography>
                                ...
                                {proveedores.length === 0 ? (
                                    ...
                                    proveedores.map((prov) => (
                                        <TableRow key={prov.id} hover>
                                            <TableCell><Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}><DemurrageIcon color="action" /><Typography fontWeight="bold">{prov.nombre}</Typography></Box></TableCell>
            ...
            {tabValue === 1 && (
                <Box>
                    <Box sx={{ display: 'flex', justifyContent: 'space-between', mb: 2, flexWrap: 'wrap', gap: 2 }}>
                        <Box>
                            <Typography variant="h6">Pagos de Demora</Typography>
                            {selectedProveedor && (
                                <Chip label={`Filtrado: ${selectedProveedor.nombre}`} onDelete={() => { setSelectedProveedor(null); fetchBolsas(); }} color="error" size="small" />
                            )}
                        </Box>
                    ...
                    {bolsas.length === 0 ? (
                        <Alert severity="info">No hay pagos de demora registrados.</Alert>
                    ) : (
            ...
            <Dialog open={proveedorDialog} onClose={() => setProveedorDialog(false)} maxWidth="sm" fullWidth>
                <DialogTitle sx={{ bgcolor: THEME_COLOR, color: 'white' }}><Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}><BusinessIcon />Nuevo Proveedor de Demora</Box></DialogTitle>
                <DialogContent sx={{ pt: 3 }}>
                    <Grid container spacing={2} sx={{ mt: 1 }}>
                        <Grid size={{ xs: 12 }}>
                            <TextField fullWidth label="Nombre del Proveedor *" value={newProveedor.nombre} onChange={(e) => setNewProveedor({ ...newProveedor, nombre: e.target.value })} placeholder="Ej: Naviera OOCL" />
                        </Grid>
                        <Grid size={{ xs: 12 }}>
                            <TextField fullWidth label="Referencia / RFC" value={newProveedor.referencia} onChange={(e) => setNewProveedor({ ...newProveedor, referencia: e.target.value })} placeholder="Ej: OOCL-001" />
                        </Grid>
                    </Grid>
                </DialogContent>
            ...
            <Dialog open={bolsaDialog} onClose={() => setBolsaDialog(false)} maxWidth="md" fullWidth>
                <DialogTitle sx={{ bgcolor: THEME_COLOR, color: 'white' }}><Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}><MoneyIcon />Registrar Pago de Demora</Box></DialogTitle>
                <DialogContent sx={{ pt: 3 }}>
            ...
            <Dialog open={deleteDialog} onClose={() => setDeleteDialog(false)} maxWidth="sm" fullWidth>
                <DialogTitle sx={{ color: 'error.main' }}>⚠️ Eliminar Pago de Demora</DialogTitle>
                <DialogContent>
                    {bolsaToDelete && (
                        <Box>
                            <Typography gutterBottom>¿Estás seguro de eliminar este pago? Se revertirá el monto de demora en los costos del contenedor.</Typography>

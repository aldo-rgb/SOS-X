...
export default function DashboardClient() {
  ...
  const getPackageTotalMXN = (pkg: PackageTracking): number => {
    ...
    // DHL (AA_DHL): el 'monto' que manda el backend YA trae todo —importación
    ...
    //
    // `national_shipping_cost` viaja aparte SOLO para poder pintar el renglón
    // del desglose, y el propio backend lo advierte donde lo manda. Sumarlo
    // aquí lo cobraba dos veces: a Sankie Guo (S105) la pantalla le mostró
    // $11,121.00 por dos cajas que valían $10,173.00, exactamente $474.00 de
    // Paquete Express repetido en cada una (TKT-2026-2876).
    //
    // Los cargos extra sí van fuera del monto y se suman.
  ...
  // Genera y abre el PDF (cotización/recibo) de una orden de pago. Compartido por
  // la tabla del modal "Mis Cuentas por Pagar" y la tabla en línea del portal.
  const downloadPaymentOrderPdf = (order: any) => {
    ...
    // Nombre legible de la paquetería (la columna mostraba la clave cruda o 'DHL').
    ...
    // La importación se saca restando, para que los renglones siempre sumen lo
    // que se está cobrando aunque el tipo de cambio traiga redondeos (tarea 282).
    const celdasDhl = (pkg: any): string => {
      ...
      return celda(fmtCur(importacion) + (usd > 0 && tc > 0 ? chica(`${usd.toFixed(2)} USD × $${tc.toFixed(2)}`) : ''))
        + celda(impuestos > 0 ? fmtCur(impuestos) : '—')
        + celda((envio > 0 ? fmtCur(envio) : '—') + (envio > 0 && paqueteria !== '—' ? chica(paqueteria) : ''))
        + celda(fmtCur(total), 'font-weight:600;');
    };
    const pkgRows = pkgs.map((pkg: any, i: number) => { const dims = (pkg.length_cm>0||pkg.width_cm>0||pkg.height_cm>0)?`${pkg.length_cm}×${pkg.width_cm}×${pkg.height_cm} cm`:'—'; const intTrk = pkg.international_tracking ? `<br><span style="font-size:9px;color:#000;">${pkg.international_tracking}</span>` : ''; const comunes = `<td style="padding:6px 8px;border-bottom:1px solid #eee;font-size:11px;">${i+1}</td><td style="padding:6px 8px;border-bottom:1px solid #eee;font-size:11px;font-weight:600;">${pkg.tracking_internal||'-'}${intTrk}</td><td style="padding:6px 8px;border-bottom:1px solid #eee;font-size:11px;text-align:center;">${pkg.weight ? Number(pkg.weight).toFixed(1)+' kg' : '—'}</td><td style="padding:6px 8px;border-bottom:1px solid #eee;font-size:11px;text-align:center;">${dims}</td>`;
      if (!isDhlQuote) return `<tr>${comunes}<td style="padding:6px 8px;border-bottom:1px solid #eee;font-size:11px;text-align:center;">${carrierLabel(pkg.national_carrier)}</td><td style="padding:6px 8px;border-bottom:1px solid #eee;font-size:11px;text-align:right;font-weight:600;">${fmtCur(montoGuia(pkg))}</td></tr>`;
      ...
      return `<tr>${comunes}${celdasDhl(pkg)}</tr>`;
    }).join('');
    ...
    const destinoPdf: string = (() => {
      const conDestino = pkgs.find((p: any) => p.destination_city || p.delivery_city);
      if (!conDestino) return '';
      const ciudad = conDestino.destination_city || conDestino.delivery_city;
      const estado = conDestino.destination_state || conDestino.delivery_state || '';
    ...
    const fmtBrk = (label: string, val: number, color?: string) => Number(val) !== 0 ? `<tr><td style="border-bottom:1px solid #f0f0f0;"></td><td colspan="${totalCols - 2}" style="padding:5px 8px;border-bottom:1px solid #f0f0f0;font-size:11px;color:${color||'#000'};">${label}</td><td style="padding:5px 8px;border-bottom:1px solid #f0f0f0;font-size:11px;text-align:right;font-weight:600;color:${color||'#000'};">${fmtCur(Number(val))}</td></tr>` : '';
                ...
                <Table size="small">
                  ...
                  <TableBody>
                    {paymentOrders.filter((o: any) => {
                      ...
                      let st = statusMap[order.status] || statusMap.pending_payment;
                      // 💳 Pagado con crédito: "Crédito" (sin liquidar) vs "Crédito Pagado" (liquidado).
                      const isCreditPay = String(order.payment_method || '').toLowerCase() === 'credit';
                      const isUnsettledCredit = isCreditPay && !order.credit_settled;
                      const isSettledCredit = isCreditPay && !!order.credit_settled;
                      if (['completed', 'paid'].includes(order.status)) {
                              ...
                              <Table size="small">
                                ...
                                <TableBody>
                                  {Array.isArray(order.packages) && order.packages.length > 0 ? order.packages.map((pkg: any, pi: number) => {
                                    ...
                                    <TableRow>
                                      <TableCell colSpan={5} sx={{ py: 1, color: '#999', fontSize: '0.75rem', textAlign: 'center' }}>Sin detalle de guías</TableCell>
                                    </TableRow>
                                  )}
                                </TableBody>
                              </Table>
